namespace ChuckDeviceController.Controllers
{
    ...
    public class ProtoController : ControllerBase
    {
        ...
        public ProtoController(IDbContextFactory<DeviceControllerContext> dbFactory, IConsumerService consumerService, ILogger<ProtoController> logger)
        {
            //_context = context;
        ...
        */
        ...
        private async Task<ProtoResponse> HandleProtoRequest(ProtoPayload payload)
        {
                            ...
                            if (gmo != null)
                            {
                                ...
                                if (wildPokemon.Count == 0 && nearbyPokemon.Count == 0 && forts.Count == 0 && quests.Count == 0)
                                {
                                    foreach (var cell in cells)
                                    {
                                        ...
                                        {
                                            _emptyCells.Add(cell, 1);
                                        }
                                        else
                                        {
                                            _emptyCells[cell]++;
                                        }
                                        if (_emptyCells[cell] == 3)
                                        {
                                            _logger.LogWarning($"[Proto] [{payload.Uuid}] Cell {cell} was empty 3 times in a row, assuming empty...");
                                            cells.Add(cell);
                                        }
                                    }
                                ...
                                {
                                    cells.ForEach(cell => _emptyCells[cell] = 0);
                                    isEmptyGmo = false;
                                }
                            }
                            else
            ...
            */
            ...
            if (total > 0)
            {
                //_logger.LogInformation($"[Proto] [{payload.Uuid}] Update count: {total} parsed in {Math.Round(stopwatch.Elapsed.TotalSeconds, 8).ToString("F16").TrimEnd('0')}s");
            }
        ...
        private async Task HandleConsumables(ConsumerData data)
        {
            ...
            if (data.Cells.Count > 0)
            {
                ...
                {
                    await _cellRepository.AddOrUpdateAsync(updatedCells);
                }

                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] S2Cell Count: {data.Cells.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
            ...
            if (data.ClientWeather.Count > 0)
            {
                ...
                foreach (var weather in data.ClientWeather)
                {
                    ...
                    updatedWeather.Add(new Weather
                    {
                        ...
                        Latitude = center.LatDegrees,
                        Longitude = center.LngDegrees,
                        GameplayCondition = 0, // TODO: (GameplayWeatherProto.Types.WeatherCondition)weather.GameplayWeather,
                        ...
                        WarnWeather = alert?.WarnWeather ?? null,
                        Updated = now,
                    });
                }
                if (updatedWeather.Count > 0)
                {
                    await _weatherRepository.AddOrUpdateAsync(updatedWeather);
                }

                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] Weather Count: {data.ClientWeather.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
            ...
            if (data.Forts.Count > 0)
            {
                ...
                {
                    await _gymRepository.AddOrUpdateAsync(updatedGyms);
                }
                if (updatedPokestops.Count > 0)
                {
                    await _pokestopRepository.AddOrUpdateAsync(updatedPokestops);
                }

                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] Fort Count: {data.Forts.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
            ...
            if (data.FortDetails.Count > 0)
            {
                ...
                foreach (var details in data.FortDetails)
                {
                    ...
                    {
                        case FortType.Gym:
                            var gym = await _gymRepository.GetByIdAsync(details.Id);
                            ...
                            break;
                        case FortType.Checkpoint:
                            var pokestop = await _pokestopRepository.GetByIdAsync(details.Id);
                ...
                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] FortDetails Count: {data.FortDetails.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
            ...
            if (data.GymInfo.Count > 0)
            {
                ...
                foreach (var gymInfo in data.GymInfo)
                {
                    ...
                    {
                        _logger.LogWarning($"[Proto] [{data.Uuid}] Invalid GymStatusAndDefenders provided, skipping...\n: {gymInfo}");
                        continue;
                    }
                ...
                {
                    await _gymRepository.AddOrUpdateAsync(updatedGyms);
                    await _trainerRepository.AddOrUpdateAsync(updatedTrainers);
                    await _gymDefenderRepository.AddOrUpdateAsync(updatedDefenders);
                }

                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] GymGetInfo Count: {data.GymInfo.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
            ...
            if (data.WildPokemon.Count > 0 || data.NearbyPokemon.Count > 0)
            {
                ...
                if (data.WildPokemon.Count > 0)
                {
                    foreach (var item in data.WildPokemon)
                    {
                        ...
                        var timestampMs = (ulong)item.timestamp_ms;
                        // TODO: HandleSpawnpoint(wildPokemon.LastModifiedMs, timestampMs);
                        var pokemon = new Pokemon(wildPokemon, cell, timestampMs, data.Username, false); // TODO: IsEvent
                ...
                if (data.NearbyPokemon.Count > 0)
                {
                    foreach (var item in data.NearbyPokemon)
                    {
                        ...
                        var pokemon = new Pokemon(nearbyPokemon, cell, data.Username, false); // TODO: IsEvent
                        ...
                        var pokestop = await _pokestopRepository.GetByIdAsync(pokemon.PokestopId);
                        if (pokestop == null)
                        {
                ...
                {
                    await _pokemonRepository.AddOrUpdateAsync(updatedPokemon);
                }
            ...
            if (data.Encounters.Count > 0)
            {
                ...
                foreach (var encounter in data.Encounters)
                {
                    ...
                    try
                    {
                        pokemon = await _pokemonRepository.GetByIdAsync(encounter.Pokemon.EncounterId.ToString()); // TODO: is_event
                    ...
                    if (pokemon != null)
                    {
                        await pokemon.AddEncounter(encounter, data.Username);
                        updatedPokemon.Add(pokemon);
                    }
                    else
                    {
                        ...
                        var newPokemon = new Pokemon(encounter.Pokemon, cellId.Id, timestampMs, data.Username, false); // TODO: IsEvent
                        await newPokemon.AddEncounter(encounter, data.Username);
            ...
            if (data.Quests.Count > 0)
            {
                ...
                foreach (var quest in data.Quests)
                {
                    ...
                    var pokestop = await _pokestopRepository.GetByIdAsync(quest.FortId);//new Pokestop(quest);
                ...
                if (updatedPokestops.Count > 0)
                {
                    await _pokestopRepository.AddOrUpdateAsync(updatedPokestops, false);
                }

                stopwatch.Stop();
                //_logger.LogInformation($"[Proto] [{data.Uuid}] Quest Count: {data.Quests.Count} parsed in {stopwatch.Elapsed.TotalSeconds}s");
            }
