namespace Chuck.Pvp
{
    ...
    public class GameMaster
    {
        ...
        {
            var data = Download(LatestGameMasterUrl);
            if (string.IsNullOrEmpty(data))
            {
                Console.WriteLine($"Failed to download latest game master from: {LatestGameMasterUrl}");
                return default;
            }
            return JsonSerializer.Deserialize<GameMaster>(data);
        }
