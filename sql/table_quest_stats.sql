...
SELECT DATE(FROM_UNIXTIME(quest_timestamp)) AS date, quest_reward_type, quest_pokemon_id, quest_item_id, 0 AS is_alternative, COUNT(*) AS count
FROM pokestop
WHERE quest_type IS NOT NULL
GROUP BY quest_reward_type, quest_pokemon_id, quest_item_id, date;

INSERT INTO quest_stats (date, reward_type, pokemon_id, item_id, is_alternative, count)
SELECT DATE(FROM_UNIXTIME(alternative_quest_timestamp)) AS date, alternative_quest_reward_type, alternative_quest_pokemon_id, alternative_quest_item_id, 1 AS is_alternative, COUNT(*) AS count
FROM pokestop
